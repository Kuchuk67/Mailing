...
from mailing.models import Attempt, ClientName, Task
...
class AnaliticView(View):
    def get(self, request, *args, **kwargs):
        ...
        count_clients_active = ClientName.objects.filter(
            unsubscribe=0, user=request.user.pk
        ).count()
        clients_unsubscribe = count_clients - count_clients_active
        # количество рассылок
        count_tasks_stop = Task.objects.filter(
            user=request.user.pk, status="stop"
        ).count()
        count_tasks = Task.objects.filter(user=request.user.pk).count()
        count_tasks_end = Task.objects.filter(
            user=request.user.pk, status="end"
        ).count()
        count_tasks_active = count_tasks - (count_tasks_end + count_tasks_stop)
        # количество отправленных писем
        email_send_ок = Attempt.objects.filter(
            task_send__user_id=request.user.pk
        ).aggregate(total=Sum("len_mail"))["total"]
        # values('len_mail').sum)
